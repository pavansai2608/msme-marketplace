...
// Multiplier applied to projected demand when recent sales dominate lifetime
// sales. Hand-chosen, not derived from any model.
const RECENT_SALES_UPLIFT = 1.4
  ...
  try {
    ...
    const totalSales = orders.reduce((acc, order) => {
      const sellerProducts = order.products.filter(
        (p) => p.seller.toString() === req.user.id.toString()
      )
    ...
    const dailyRevenue = Object.keys(dailyData)
      .sort((a, b) => a - b)
      .map((day) => ({
        day: parseInt(day),
    ...
    console.log(
      `[Stats] Month: ${currentMonth + 1}, Year: ${currentYear}, Orders found: ${currentMonthOrders.length}`
    )
  ...
  try {
    ...
    const forecastAnalytics = products.map((product) => {
      ...
      const recentSales = productSales
        .filter((s) => s.date >= last30Days)
        .reduce((acc, s) => acc + s.qty, 0)

      // 1. Demand Forecasting (7, 15, 30 days)
      const avgDailyDemand =
        totalSold /
        Math.max(30, (dateNow - (product.createdAt || last30Days)) / (1000 * 60 * 60 * 24))

      // If more than half of all sales happened in the last 30 days, demand is
      // rising, so the projection is scaled up. This is a flat heuristic
      // multiplier chosen by hand - not a fitted or learned parameter.
      const trendMultiplier = recentSales > totalSold / 2 ? RECENT_SALES_UPLIFT : 1.0
      ...
      const suggestions = []
      if (recentSales > 10)
        suggestions.push('Fast-moving item detected. Consider a 5% price premium.')
...
exports.generateWaybill = async (req, res) => {
  try {
    ...
    if (!isSeller) {
      return res
        .status(403)
        .json({ success: false, message: 'Not authorized to dispatch this order' })
    }
    ...
    res
      .status(200)
      .json({ success: true, trackingId: awb, message: 'Waybill generated via Shiprocket Mock' })
  } catch (err) {
    res.status(500).json({ success: false, message: err.message })
  }
}
...
exports.trackOrder = async (req, res) => {
  try {
    ...
    if (!order)
      return res
        .status(404)
        .json({ success: false, message: 'No order found with this tracking ID' })
