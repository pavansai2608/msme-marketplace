...
exports.getWishlist = async (req, res) => {
  try {
    ...
    console.log(
      `[API] getWishlist completed in ${Date.now() - start}ms. Found ${wishlist.length} items`
    )
...
exports.addAddress = async (req, res) => {
  try {
    const { name, phone, pincode, locality, street, city, state, landmark, altPhone, type } =
      req.body
    const user = await User.findById(req.user.id)
...
exports.deleteAddress = async (req, res) => {
  try {
    ...
    console.log(
      `Current address IDs:`,
      user.savedAddresses.map((a) => a._id.toString())
    )
